...
mod_mortality_calc_ui <- function(id) {
  ...
  tagList(
    fluidRow(
      ...
      column(
        8,
        h4("Derivation breakdown"),
        DTOutput(ns("derivation_table")),
        br(),
        h4("Mortality table preview"),
        helpText("Upload a mortality table on the Data Inputs tab. The default built-in table is shown below."),
...
mod_mortality_calc_server <- function(id, n_productions, production_duration_days, exposure_years, mortality_data) {
  moduleServer(id, function(input, output, session) {
    ...
    computed <- reactive({
      ...
      req(n_productions(), production_duration_days(), exposure_years())
      ...
      n_actors_total <- n_productions() * input$avg_key_actors
      ...
      annual_freq <- pmax(expected_total / exposure_years(), 1e-8)
    ...
    output$derivation_table <- renderDT({
      res <- computed()
      tbl <- data.frame(
        Step = c(
      ...
      datatable(tbl, rownames = FALSE, options = list(dom = "t", autoWidth = TRUE))
    })
